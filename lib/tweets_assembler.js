...
var Renderer = {
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(tweet.in_reply_to_status_id) {
      ...
      ClickEmulator.handleClick(inReply, function() {
        var target =$(inReply).parents("div[tweetid]");
        expandInReplay(tweet,target);
      });
    }
...
function expandInReplay(tweet,target)
{
  if(target.next().attr("expanded")==="true"){
    return;
  }
  $("#queue_loading").show();
  tweetManager.getTweet(function(success,data,status){
    if(success){
      if(target.next().attr("expanded")==="true"){
        return;
      }
      target.after(Renderer.renderTweet(data));
      target.next().attr("expanded","true")
      $("#queue_loading").hide();
    }
