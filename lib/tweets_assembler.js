...
var Renderer = {
  ...
  transformTweetText: Transforms.transformFactory([
      ...
      },
      {
        ...
        'replacement': function() {
          ...
          link.onclick = function() { openTab(url); };
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(tweet.source) {
      ...
      from = Renderer.makeSpan(tweetSource.replace(/(href=(['"])(.*?)\2)/i, "href='$3' onclick=\"openTab('$3')\""));
    }
    if(tweet.geo) {
      ...
      geo = " <a href=\"" + href + "\" onclick=\"openTab('";
      geo += href;
      geo += "')\"";
    ...
    if(tweet.retweeted_status) {
      ...
      img2.onclick = function() {
        openTab(TwitterLib.URLS.BASE + tweet.user.screen_name);
      };
...
function openTab(tabUrl) {
  ...
  if(event) {
    ...
    if(event.button == 1) {
      background = true;
    } else if(event.hasOwnProperty('metaKey')) {
      if(event.metaKey) {
        background = true;
      }
  ...
  var target = '_top';
  if(background || Renderer.isDesktop()) {
    target = '_blank';
  }
  var obj = window.open(tabUrl, target);
  if(background && obj) {
    obj.blur();
  }
}
