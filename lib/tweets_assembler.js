...
var Renderer = {
  ...
  transformTweetText: function(oldText) {
    var transformList = [
      ...
      },
      {
        //create users links
        'expression': /(@)(\w*)/ig,
        'replacement': "$1<a href='" + TwitterLib.URLS.BASE + "$2')\">$2</a>"
      }
    ];
  ...
  renderTweet: function (tweet) {
    ...
    var tweetUserName = user[this.nameAttribute];
    var tweetTitleUserName;
    if(this.nameAttribute.indexOf('screen') == 0) {
  ...
  assemblyTweets: function (tweets) {
    ...
    if(existingDestination.length == 1) {
      ...
      destination.slideDown('fast', function(){
        $(this).fadeOut(3000, removeElement).hover(
          function() {
            $(this).stop().show().css('opacity', '1.0');
          },
          function() {
            $(this).fadeOut(3000, removeElement);
          }
        );
      });
    }
...
if(window == top) {
  chrome.extension.sendRequest({cb_requesting_tweets: true}, function(response) {
