...
var Renderer = {
  ...
  transformTweetText: Transforms.transformFactory([
      ...
      },
      {
        ...
        function() {
          ...
          var link = Renderer.makeElem("a", {href: '#'});
          ClickEmulator.handleClick(link, function(event) {
            Renderer.handleUserClick(username, TwitterLib.URLS.BASE + username);
          });
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    var openUserProfile = function(screenName) {
      screenName = screenName || user.screen_name;
      return function() {
        Renderer.handleUserClick(screenName, TwitterLib.URLS.BASE + screenName);
      };
    };
    ...
    if(tweet.retweeted_status) {
      ...
      ClickEmulator.handleClick(img2, openUserProfile(tweet.user.screen_name));
