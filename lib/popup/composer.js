var Composer = {
  ...
  textareaChanged: function (e) {
    ...
    var composeButtonEl = $("#compose_tweet_area input[type='button']");
    var isDMS = tweetManager.currentTimelineId == TimelineTemplate.DMS;
    if (isDMS) {
      charsLeftEl.hide();
      composeButtonEl.removeAttr("disabled");
    } else {
      ...
      if(availableChars < 0 || availableChars == MAX_TWEET_SIZE) {
        ...
        composeButtonEl.attr("disabled", "disabled");
      } else {
    ...
    if(!composeButtonEl.is(":disabled") && (e && e.ctrlKey && e.which == 13)) { // Ctrl + Enter
      this.sendTweet();
    }
  },
