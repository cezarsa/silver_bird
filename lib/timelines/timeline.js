...
TweetsTimeline.prototype = {
  ...
  _onFetchNew: function(success, tweets, status, context) {
    ...
    var shouldSync = this._stageNewTweets(tweets);
    if(shouldSync) {
      var newTweetsLen = this._syncNewTweets(context);
      if(newTweetsLen > 0) {
        this.manager.notifyNewTweets();
      }
      if(context.onFinish) {
        context.onFinish(newTweetsLen);
      }
      this.timerId = setTimeout(function() { _this._fetchNewTweets(); }, this.template.refreshInterval);
    } else {
      this._fetchNewTweets(context.onFinish);
    }
  },
  ...
  _makeNewTweetsRequestParams: function() {
    var lastId = null;
    if(this.newTweetsCache.length > 1) {
      lastId = this.newTweetsCache[1].id;
    } else {
      var requiredLen = 1 - this.newTweetsCache.length;
      if(this.tweetsCache.length > requiredLen) {
        lastId = this.tweetsCache[requiredLen].id;
      }
    }

    var maxId = null;
    if(this.stagedNewTweets.length > 0) {
      maxId = this.stagedNewTweets[this.stagedNewTweets.length - 1].id;
    }
  ...
  _stageNewTweets: function(tweets) {
    ...
    if(this.stagedNewTweets.length > 0 &&
        this.stagedNewTweets[this.stagedNewTweets.length - 1].id == tweets[0].id) {
      tweets.shift();
      if(tweets.length === 0) {
        return true;
      }
    }
    if(this.stagedNewTweets.length > 0 && this.stagedNewTweets[0].id == tweets[0].id &&
        this.stagedNewTweets[this.stagedNewTweets.length - 1].id == tweets[tweets.length - 1].id) {
      return true;
    }

    this.stagedNewTweets = this.stagedNewTweets.concat(tweets);

    var lenCache = this.tweetsCache.length;
    var lenNewCache = this.newTweetsCache.length;
    if(lenCache === 0 && lenNewCache === 0) {
      return true;
    }
    var mostRecentId = lenNewCache > 0 ? this.newTweetsCache[0].id : this.tweetsCache[0].id;
    if(this.stagedNewTweets[this.stagedNewTweets.length - 1].id == mostRecentId) {
      this.stagedNewTweets.pop();
      return true;
    }
    if(this.stagedNewTweets.length >= 250) { //TODO: Hardcoded limit for now, maybe an option?
      return true;
    }
    return false;
  },
  ...
  _syncNewTweets: function(context) {
    ...
    var stagedLen = tweets.length;
    for(var i = stagedLen - 1; i >= 0; --i) {
