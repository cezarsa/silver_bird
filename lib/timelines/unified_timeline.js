function UnifiedTweetsTimeline(timelineId, manager, template) {
  TweetsTimeline.call(this, timelineId, manager, template);
  this.timelines = manager.timelines;
  this.currentPage = 0;
}
...
$.extend(UnifiedTweetsTimeline.prototype, TweetsTimeline.prototype, {
  /* overridden */
  init: function() {
    TimelineTemplate.eachTimelineTemplate(function(timeline) {
      timeline.loadOptions();
    });
  },
  ...
  _eachTimeline: function(callback) {
    for(var tId in this.timelines) {
      ...
      if(timeline.template.includeInUnified) {
