function TwitterLib(requestTimeout, onAuthenticated, onHitsUpdated, microbloggingService, baseUrl, baseOauthUrl, baseSigningUrl, baseOauthSigningUrl, baseSearchUrl, oauthTokenData) {
...
TwitterLib.prototype = {
  ...
  ajaxRequest: function(url, callback, context, params, httpMethod, useSearchAPI) {
    if(!httpMethod)
      httpMethod = "GET";
    if(!params)
      params = {};
    ...
    $.ajax({
      ...
      success: function(data, status) {
        //console.log(requestUrl + '?' + $.param(requestParams), data);
        if(!data) {
          data = [];
  ...
  tweet: function(callback, msg, replyId) {
    ...
    var oldTimeoutValue = this.requestTimeout;
    this.requestTimeout = 30000;
    this.ajaxRequest('statuses/update', callback, null, params, "POST");
    this.requestTimeout = oldTimeoutValue;
  },
