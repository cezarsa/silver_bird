function TwitterLib(requestTimeout, onAuthenticated) {
...
TwitterLib.URLS = {
  ...
  OAUTH_AUTHORIZE: 'https://twitter.com/oauth/authorize?oauth_token='
};
  ...
  ajaxRequest: function(url, callback, context, params, httpMethod) {
    ...
    $.ajax({
      ...
      complete: function(request) {
        if(!request) return;
        try {
          _this.remainingHitsCount = request.getResponseHeader("X-RateLimit-Remaining");
