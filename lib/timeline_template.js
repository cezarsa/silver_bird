function TimelineTemplate(timelineTemplateId, tweetManager) {
  ...
  this.initTemplate = function() {
    ...
    // timelineName resources will be processed later on, 
    // because at this point we don't have locales loaded yet
    switch(this.id) {
      case TimelineTemplate.UNIFIED:
        this.timelineName = "w_Unified";
        this.factory = new UnifiedTimelineFactory(this);
        break;
      case TimelineTemplate.HOME:
        this.timelineName = "w_Home";
...
TimelineTemplate.getTimelineTitleLocale = function(tlId, tlName) {
  // gets locale string if it's the case
  if (TimelineTemplate.timelineNames.indexOf(tlId) >= 0 && tlId != TimelineTemplate.MENTIONS) {
    return chrome.i18n.getMessage(tlName);
  }
  else return tlName;
}
